...
**!!!每个语句只能被 build 一次，否则可能导致出现不应该有的副作用!!!**
比如 a = f() % b，这个按原来的写法，f()会被 build 两次，这样就执行了两次函数调用！！！
...
注意符号表里的维数要慎用，因为 llvm 系列对象中最高只有 1 维数组
...
1. 先搞定全局变量
2. 之后遍历 llvm 树，从其他结点生成汇编
3. 每次alloca的时候先来个li 0

## 提醒

- 尽量保证除了构造方法以外的其他方法都是纯函数，因为作为构造方法传入的参数在外面可能还是要用的，而构造方法通常只是浅克隆复制了引用，这样一改对象内部的成分，外面的对象也会变
- 检查一下方法引用时候使用正确，看是否存在子类使用成父类方法的情况
- 之前说会卡 llvm ir 的 tle，那 mips 也会卡吗？
- .sapce声明的空间可以认为是0吗
- 注意哪里可能用到全局变量，要在对应的位置加la
